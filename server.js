...
// NON-BLOCK helper - returns boolean (for use with #if)
hbs.registerHelper('contains', function(array, value) {
    if (!array || !Array.isArray(array)) return false;
    return array.indexOf(value) !== -1;
});

// BLOCK helper - for conditional rendering {{#ifContains}}{{/ifContains}}
...
const { upload, uploadToCloudinary, deleteFromCloudinary, getPublicIdFromUrl } = cloudinaryConfig;

// Upload course image
app.post('/api/upload/course-image', isAuthenticated, isAdmin, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
...
app.post('/api/upload/team-image', isAuthenticated, isAdmin, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No image file provided' });
        }
...
app.post('/api/upload/project-image', isAuthenticated, isAdmin, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No image file provided' });
        }
...
// Upload blog image
app.post('/api/upload/blog-image', isAuthenticated, isAdmin, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No image file provided' });
        }
...
app.delete('/api/upload/delete', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
app.get('/learn', async (req, res) => {
    try {
        ...
        const stats = statsR.rows[0] || { total_courses: 0, total_categories: 0, total_enrollments: 0, total_instructors: 0 };
...
app.get('/learn/course/:id', async (req, res) => {
    try {
        ...
        // Get course details - FIXED VERSION
        ...
        const totalLessons = modules.reduce((sum, m) => sum + (m.lessons?.filter(l => l && l.id)?.length || 0), 0);
...
app.post('/api/enroll', isAuthenticated, async (req, res) => {
    try {
        ...
        try {
            await db.query(
                `INSERT INTO user_activities (user_id, activity, type, date)
                 VALUES ($1, $2, 'enrollment', NOW())`,
                [userId, `Enrolled in ${course.title}`]
            );
        } catch (logErr) {
        ...
        // Get updated enrollment count for stats
...
app.get('/learn/course/:id/dashboard', isAuthenticated, async (req, res) => {
    try {
        ...
        // Verify enrollment
        const eR = await db.query(
            `SELECT e.*, c.title, c.description, c.image_url, c.category, c.level
             FROM enrollments e
             JOIN courses c ON e.course_id = c.id
        ...
        res.render('course-dashboard', {
            ...
            courseImage: enrollment.image_url,
...
app.get('/my-learning', isAuthenticated, async (req, res) => {
    try {
        ...
        // Calculate stats for enrolled courses
...
// API endpoint to get user's enrollment status for a course
